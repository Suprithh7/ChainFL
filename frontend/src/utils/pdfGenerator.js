...
export const generatePatientPDF = (patient, editedData = {}) => {
  try {
    ...
    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    doc.text('CARDIAC RISK', 20, yPos + 8);
    doc.setFontSize(18);
    ...
    if (patient.multi_disease_risks) {
      ...
      diseases.forEach(([disease, data], index) => {
        ...
        doc.setFontSize(10);
        doc.setFont('helvetica', 'bold');
        doc.text(`${diseaseIcons[disease]}`, xPos + 3, yPos + yOffset + 7);
        doc.setFontSize(14);
    ...
    if (patientInfo.doctor_notes && patientInfo.doctor_notes.trim()) {
      // Check if we need a new page
      if (yPos + 40 > doc.internal.pageSize.getHeight()) {
        doc.addPage();
        yPos = 20;
      }
      ...
      const notes = doc.splitTextToSize(patientInfo.doctor_notes, pageWidth - 35);
      const notesHeight = notes.length * 5 + 10;
      doc.roundedRect(10, yPos - 5, pageWidth - 20, notesHeight, 2, 2, 'F');
